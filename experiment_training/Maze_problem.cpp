...
vector<pair<int,int>> path_stack;//DFS
deque<pair<int,int>> path_queue;//BFS
vector<vector<pair<int,int>>> parent;//path recursive,record parent position
...
{
    pair<int,int> current; current.first=row,current.second=col; path_queue.push_back(current); maze_map[row][col]=2;
    while(!path_queue.empty())
    {
        current=path_queue.front(); path_queue.pop_front();
        if(current.first==out.first && current.second==out.second){break;}  //found out
...
int main(void)
{
    ...
    for(pair output:path_stack) {cout <<'(' << output.first << ',' << output.second << ')' << ' ';} cout << endl;
