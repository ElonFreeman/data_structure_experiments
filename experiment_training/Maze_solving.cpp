...
bool findpath_DFS(int row,int col)
{
    ...
    {
        pair<int,int> back; back.first=row,back.second=col; path_stack.push_back(back);
        return true;
    }
...
void findpath_BFS(int row,int col)
{
    ...
    while(!path_queue.empty())
    {
        ...
        for(int i=0;i<4;i++)
        {
            ...
            {
                pair<int,int> neighbor={cx,cy}; path_queue.push_back(neighbor); 
                maze_map[cx][cy]=2; parent[cx][cy]=current; 
            }
        }
    }
}
